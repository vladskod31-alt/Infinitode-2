/* INFINITODE 4 — UI layer: screens, HUD, i18n, input, main loop */
(function () {
  'use strict';
  ...
  const UI = {
    game: null, renderer: null, endlessMode: false, mapId: 'valley',
    ...
    renderMaps() {
      ...
      for (const m of globalThis.BAL.MAPS) {
        const won = globalThis.STORE.S.stats.mapsWon.includes(m.id);
        const card = document.createElement('button');
        card.className = 'map-card';
        card.style.setProperty('--accent', m.accent);
        card.innerHTML = `<img src="${m.bg}" alt=""/><div class="map-info"><b>${TN(m.name)} ${won ? '★' : ''}</b><span>${TN(m.desc)}</span></div>`;
    ...
    startGame(mapId) {
      this.mapId = mapId;
      const rfx = globalThis.BAL.researchFx(globalThis.STORE.S.research.levels);
      this.game = new globalThis.Game(mapId, { rfx, endless: this.endlessMode, events: (t, d) => this.onEvent(t, d) });
    ...
    onGameOver(d) {
      ...
      S.save();
      if (S.S.stats.mapsWon.length >= 3) { const def = S.unlockAch('win_all'); if (def && def.name) setTimeout(() => this.achPopup(def), 1500); }
      d.win ? globalThis.AUDIO.SFX.victory() : globalThis.AUDIO.SFX.defeat();
      globalThis.AUDIO.playMusic('menu');
      const mm = Math.floor(d.time / 60), ss = Math.floor(d.time % 60);
      this.openModal(`
        <h2 class="${d.win ? 'win' : 'lose'}">${d.win ? T('victory') : T('defeat')}</h2>
    ...
    refreshTowerPanel() {
      ...
      const need = tw.level >= 10 ? 1 : globalThis.BAL.xpNeed(tw.level);
      $('tp-xp').style.width = `${tw.level >= 10 ? 100 : Math.min(100, (tw.xp / need) * 100)}%`;
      const ab = globalThis.BAL.ABILITIES[tw.type];
      ...
      if (tw.level >= 10) { $('tp-up').innerHTML = T('maxed'); $('tp-up').disabled = true; }
    ...
    openAbilityModal() {
      ...
      this.game.paused = true;
      const [a, b] = [globalThis.BAL.ABILITIES[q.tw.type][q.tier * 2], globalThis.BAL.ABILITIES[q.tw.type][q.tier * 2 + 1]];
      this.openModal(`
    ...
    renderProfile() {
      ...
      $('pf-stats').innerHTML = [
        ...
        [T('earned'), st.earned], [T('strikes'), st.strikes], [T('mapsWon'), st.mapsWon.length + '/3'],
    ...
    renderResearch() {
      const S = globalThis.STORE;
      $('rp-points').textContent = `🔬 ${S.S.research.points} ${T('researchPts')}`;
      const list = $('res-list');
      list.innerHTML = '';
      for (const r of globalThis.BAL.RESEARCH) {
        const lv = S.S.research.levels[r.id] || 0;
        const maxed = lv >= r.max;
        const cost = maxed ? 0 : r.costs[lv];
        const can = !maxed && S.S.research.points >= cost;
        const d = document.createElement('div');
        d.className = 'res-row';
        ...
        if (can) d.querySelector('button').onclick = () => {
          globalThis.AUDIO.SFX.upgrade();
          S.S.research.points -= cost;
          S.S.research.levels[r.id] = lv + 1;
