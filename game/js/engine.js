/* INFINITODE 4 — game engine (logic + simulation). Depends on BAL, STORE, AUDIO. */
(function () {
  'use strict';
  ...
  class Game {
    constructor(mapId, opts) {
      ...
      this.lives = this.RFX.maxLives;
      this.maxLives = this.RFX.maxLives;
      this.wave = 0;
      this.score = 0;
      this.time = 0;
      this.speed = 1;
    ...
    chooseAbility(tw, idx) {
      ...
      const ab = this.BAL.ABILITIES[tw.type][tier * 2 + idx];
      if (ab) { ab.apply(tw); tw.abilities.push(ab.id); }
      this.abilityQueue.splice(q, 1);
      this.emit('towersChanged', {});
    }
    addXp(tw, n) {
      if (tw.level >= 10) return;
      tw.xp += n * this.RFX.xpMul;
      while (tw.level < 10 && tw.xp >= this.BAL.xpNeed(tw.level)) {
    ...
    spawnEnemy(type) {
      ...
      const hp = Math.round(d.hp * this.BAL.hpMul(this.wave));
    ...
    damageEnemy(e, amount, tw, opts) {
      ...
      if (e.hp <= 0) return 0;
      let dmg = amount;
      if (tw && tw.m.crit && !opts.noCrit && Math.random() < tw.m.crit.ch) { dmg *= tw.m.crit.mul; this.floater(e.x, e.y - 20, 'CRIT!', '#ffe27a', 15); }
    ...
    killEnemy(e, tw) {
      e.hp = 0;
      this.kills++;
      let rw = Math.round(e.d.reward * this.BAL.rewardMul(this.wave) * (tw && tw.m.bounty ? 1 + tw.m.bounty : 1));
    ...
    explode(x, y, radiusPx, dmg, tw, airCap) {
    ...
    tryFire(tw, dt) {
      ...
      switch (tw.type) {
        case 'miner': {
          ...
          if (tw.tick >= period) {
            tw.tick -= period;
            const gain = Math.round(def.income * (1 + 0.25 * (tw.level - 1)) * (tw.m.incMul || 1));
            this.coins += gain; this.earned += gain;
        ...
        if (p.kind === 'homing') {
          ...
          if (p.target) {
            ...
            if (dd < p.target.d.r + 10) {
              this.explode(p.target.x, p.target.y, p.splash, p.dmg, p.tw, p.air);
              globalThis.AUDIO.SFX.boom();
              this.projs = this.projs.filter(x => x !== p);
              continue;
            }
