/* INFINITODE 4 — balance data & pure logic. No DOM. Works in browser + Node. */
(function () {
  'use strict';
  ...
  const TOWERS = {
    ...
    heli:    { order: 17, kind: 'heli',    cost: 420, dmg: 14,  rate: 2.0,  range: 5.0, air: 1, color: '#ffd968', drones: 2,
    ...
    miner:   { order: 18, kind: 'miner',   cost: 150, dmg: 0,   rate: 0.2,  range: 0,   air: 0, color: '#5ef2b8', income: 12.5,
  ...
  const ENEMIES = {
    ...
    boss:    { hp: 4200, speed: 0.55, reward: 300, flying: false, r: 30, color: '#ef4444', armor: 5, dmg: 5, boss: true, stunImmune: true, name: { uk: 'БРУТ (бос)', en: 'BRUTE (boss)' } },
  };
  ...
  function genWave(w) {
    ...
    if (w === 1) push('regular', 8, 1.1, 0.5);
    ...
    else if (w === 6) { push('armored', 6, 1.2, 0.5); push('fast', 12, 0.55, 1); }
    ...
    else if (w === 11) { push('healer', 4, 2.0, 4); push('strong', 10, 0.9, 0.3); push('regular', 16, 0.45, 0); }
    ...
    else if (w === 16) { push('icy', 10, 0.9, 0.3); push('strong', 10, 0.8, 1); }
      ...
      const k = w - 30;
      const pool = ['regular', 'fast', 'strong', 'armored', 'healer', 'toxic', 'icy', 'fighter', 'light', 'heli', 'jet'];
      ...
      if (w % 5 === 0) push('boss', 1 + Math.floor(k / 10), 7, 3);
    }
  ...
  const MAPS = [
    { id: 'valley', name: { uk: 'Зелена долина', en: 'Green Valley' }, bg: 'assets/img/bg-map1.png', accent: '#39f5c8',
      ...
      wp: [[-1, 2], [5, 2], [5, 8], [10, 8], [10, 2], [15, 2], [15, 8], [20, 8]] },
    { id: 'desert', name: { uk: 'Піщаний вир', en: 'Sand Vortex' }, bg: 'assets/img/bg-map2.png', accent: '#ffb52e',
      ...
      wp: [[-1, 10], [3, 10], [3, 2], [16, 2], [16, 6], [7, 6], [7, 8], [13, 8], [13, 5], [10, 5]] },
    { id: 'arctic', name: { uk: 'Арктична ніч', en: 'Arctic Night' }, bg: 'assets/img/bg-map3.png', accent: '#7db4ff',
      ...
      wp: [[-1, 1], [18, 1], [18, 4], [1, 4], [1, 7], [18, 7], [18, 10], [20, 10]] },
  ];
  ...
  function researchFx(levels) {
    const L = (id) => (levels && levels[id]) || 0;
    return {
      dmgMul: 1 + 0.08 * L('dmg'), rateMul: 1 + 0.06 * L('rate'), rangeMul: 1 + 0.05 * L('range'),
  ...
  const ACH = [
    ...
    { id: 'win_all', xp: 250, rp: 5, name: { uk: 'Захисник світу', en: 'World defender' }, desc: { uk: 'Пройди всі 3 мапи', en: 'Beat all 3 maps' } },
